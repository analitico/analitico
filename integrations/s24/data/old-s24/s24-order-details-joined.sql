# query containing a list of all order details used to train the model
# that predicts the time necessary to go from a category to the next
# and reorder the picking list for the couriers

select 
	ord.id 'order_id', detail.id 'detail_id', 
    store.id 'store_id', store.name 'store_name', store.area 'store_area', store.ref_id 'store_ref_id',
