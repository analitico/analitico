...
def _eval_input_receiver_fn(schema):
  ...
  """
    ...
    return tfma.export.EvalInputReceiver(
        features=features, receiver_tensors=receiver_tensors, labels=features.pop(_LABEL_KEY)
    )
...
def trainer_fn(hparams, schema):
  ...
  """
    ...
    train_input_fn = lambda: _input_fn(  # pylint: disable=g-long-lambda
        hparams.train_files, schema, batch_size=train_batch_size
    )

    eval_input_fn = lambda: _input_fn(  # pylint: disable=g-long-lambda
        hparams.eval_files, schema, batch_size=eval_batch_size
    )

    train_spec = tf.estimator.TrainSpec(train_input_fn, max_steps=hparams.train_steps)  # pylint: disable=g-long-lambda
    ...
    eval_spec = tf.estimator.EvalSpec(eval_input_fn, steps=hparams.eval_steps, exporters=[exporter], name="iris-eval")
