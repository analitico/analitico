...
class K8ViewSetMixin:
    ...
    def metrics(self, request, pk, stage):
        ...
        metrics = {
            "istio_requests_total": f'istio_requests_total{{destination_service_namespace="{service_namespace}", destination_service_name=~"{service_name}"}}',
            "istio_request_duration_seconds_count": f'istio_request_duration_seconds_count{{destination_service_namespace="{service_namespace}", destination_service_name=~"{service_name}"}}',
            "istio_request_duration_seconds_sum": f'istio_request_duration_seconds_sum{{destination_service_namespace="{service_namespace}", destination_service_name=~"{service_name}"}}',
            "istio_requests_rate": f'rate(istio_requests_total{{destination_service_namespace="{service_namespace}", destination_service_name=~"{service_name}"}}[1m])',
            "istio_request_latency": (
                f'rate(istio_request_duration_seconds_sum{{destination_service_namespace="{service_namespace}", destination_service_name=~"{service_name}"}}[1m]) / '
                f'rate(istio_request_duration_seconds_count{{destination_service_namespace="{service_namespace}", destination_service_name=~"{service_name}"}}[1m])'
            ),
    ...
    @action(
        methods=["get"], detail=True, url_name="k8-job-logs", url_path=r"k8s/web/(?P<stage>staging|production)/logs"
    )
    def web_logs(self, request: Request, pk, stage):
        ...
        query += " AND kubernetes.labels.serving_knative_dev\/service.keyword:" + (
            "/.*-staging/" if stage == "staging" else "(NOT staging)"
        )
