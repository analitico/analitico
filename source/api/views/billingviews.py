...
class BillingViewSet(ViewSet):
    ...
    def get_stripe_response(self, items, item_type, many=False, status=status.HTTP_200_OK):
        if many:
    ...
    def get_plans(self, request: Request):
        ...
        return self.get_stripe_response(plans, "analitico/stripe-plan", many=True)
    ...
    def create_session(self, request: Request):
        ...
        if not plan:
            msg = "Please provide ?plan= to be purchased with checkout."
        ...
        return self.get_stripe_response(session, "analitico/stripe-session", status=status.HTTP_201_CREATED)
    ...
    def get_invoices(self, request: Request, pk: str):
        ...
        return self.get_stripe_response(invoices, "analitico/stripe-invoice", many=True)

    @action(methods=["get", "delete"], detail=True, url_name="subscription", url_path="subscription")
    def subscription(self, request: Request, pk: str):
        """ Returns the current subscription on the given workspace or cancels it. """
        workspace = factory.get_item(pk)
        ...
        if request.method == "DELETE":
            ...
            return self.get_stripe_response(subscription, "analitico/stripe-subscription")
    ...
    def webook(self, request: Request):
        ...
        if (event_livemode is False) and not ("_test_" in settings.ANALITICO_STRIPE_SECRET_KEY):
