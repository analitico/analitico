...
class K8ViewSet(GenericViewSet):
    ...
    def metrics(self, request, pk):
        """ Returns a list of metrics owned by the service """
        service_name, service_namespace = ("ep-test-001", "cloud") #self.get_service_name(request, pk)
        ...
        if not search(braces, query):
            # TODO: is this the proper exception?
            raise AnaliticoException("Metrics must be filtered by service", status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)
        
        query = query.replace("{", '{kubernetes_namespace="%s",serving_knative_dev_service="%s",' % (service_namespace, service_name))

        prometheusResponse = requests.post("https://prometheus.cloud.analitico.ai/api/v1/query", data={"query": query}, headers={"Content-Type": "application/x-www-form-urlencoded"})
        
        return Response(prometheusResponse.json(), content_type="json")

    @action(methods=["get"], detail=True, url_name="logs", url_path="logs")
    def logs(self, request, pk):
        """ Returns logs generated by the service """
        # TODO: 
        service_name, service_namespace = ("ep-test-001", "cloud") #self.get_service_name(request, pk)
        ...
        url = "https://cloud.analitico.ai:6443/api/v1/namespaces/knative-monitoring/services/elasticsearch-logging/proxy/_search" 
        token = django.conf.settings.PROMETHEUS_API_TOKEN
        ...
        prometheus_response = requests.get(url, params=params, headers=headers, verify=False)
        
        return Response(prometheus_response.json(), content_type="json")
