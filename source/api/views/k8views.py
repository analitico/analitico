...
class K8ViewSet(GenericViewSet):
    ...
    @action(methods=["get"], detail=True, url_name="metrics", url_path=r"metrics/(?P<stage>staging|production)$")
    ...
    @action(methods=["get"], detail=True, url_name="logs", url_path=r"logs/(?P<stage>staging|production)$")
    def logs(self, request, pk, stage):
        """ 
