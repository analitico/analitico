...
class ProjectsApiTests(api.test.APITestCase):
    ...
    def test_project_api_with_existing_project(self):
        ...
        response = self.client.get("/api/projects/test-project1", format="json")
        self.assertIsNotNone(response.data)
        settings = response.data["settings"]
        self.assertIsNotNone(settings)
        self.assertEqual(
            settings["data"]["parameters"]["learning_rate"], PRJ_SETTINGS["data"]["parameters"]["learning_rate"]
        )
    ...
    def test_project_api_with_existing_project_no_token(self):
        self.auth_token()
        response = self.client.get("/api/projects/s24-order-time", format="json")
    ...
    def test_project_api_with_missing_project(self):
        self.auth_token(self.token1)
        response = self.client.get("/api/projects/fake-id", format="json")
