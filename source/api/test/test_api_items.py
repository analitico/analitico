...
class ItemsTests(AnaliticoApiTestCase):
    ...
    def test_item_clone_as_regular_user_with_permission(self):
        # recipe to clone
        item = api.models.Recipe(workspace=self.ws1)
        item.save()
        # user with editor permissions
        role = api.models.Role(workspace=self.ws1, user=self.user2)
        role.roles = "analitico.editor"
        role.save()

        self.auth_token(self.token2)
        url = reverse(f"api:recipe-clone", args=(item.id,))
