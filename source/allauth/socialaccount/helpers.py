...
def render_authentication_error(request, provider_id, error=AuthError.UNKNOWN, exception=None, extra_context=None):
    try:
    ...
    context = {"auth_error": {"provider": provider_id, "code": error, "exception": exception}}
    context.update(extra_context)
    return render(request, "socialaccount/authentication_error." + account_settings.TEMPLATE_EXTENSION, context)
...
def _add_social_account(request, sociallogin):
    ...
    if sociallogin.is_existing:
        if sociallogin.user != request.user:
            ...
            action = "updated"
            signals.social_account_updated.send(sender=SocialLogin, request=request, sociallogin=sociallogin)
    else:
    ...
    get_account_adapter(request).add_message(
        request, level, message, message_context={"sociallogin": sociallogin, "action": action}
    )
