...
from ..account.views import AjaxCapableProcessFormViewMixin, CloseableSignupMixin, RedirectAuthenticatedUserMixin
...
class ConnectionsView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    def form_valid(self, form):
        get_account_adapter().add_message(
            self.request, messages.INFO, "socialaccount/messages/" "account_disconnected.txt"
        )
        form.save()
    ...
    def get_ajax_data(self):
        ...
        for account in SocialAccount.objects.filter(user=self.request.user):
            ...
            account_data.append({"id": account.pk, "provider": account.provider, "name": provider_account.to_str()})
