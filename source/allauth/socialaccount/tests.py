...
class OAuthTestsMixin(object):
    ...
    def test_login(self):
        ...
        data = dict(email=user_email(sociallogin.user), username=str(random.randrange(1000, 10000000)))
    ...
    @override_settings(SOCIALACCOUNT_AUTO_SIGNUP=True, SOCIALACCOUNT_EMAIL_REQUIRED=False, ACCOUNT_EMAIL_REQUIRED=False)
    ...
    def login(self, resp_mocks, process="login"):
        with mocked_response(
            MockedResponse(200, "oauth_token=token&" "oauth_token_secret=psst", {"content-type": "text/html"})
        ):
            resp = self.client.get(reverse(self.provider.id + "_login"), dict(process=process))
    ...
    def get_access_token_response(self):
        return MockedResponse(200, "oauth_token=token&oauth_token_secret=psst", {"content-type": "text/html"})

    def test_authentication_error(self):
        resp = self.client.get(reverse(self.provider.id + "_callback"))
        self.assertTemplateUsed(
            resp, "socialaccount/authentication_error.%s" % getattr(settings, "ACCOUNT_TEMPLATE_EXTENSION", "html")
        )
...
class OAuth2TestsMixin(object):
    ...
    def test_account_tokens(self, multiple_login=False):
        ...
        if multiple_login:
            self.login(self.get_mocked_response(), with_refresh_token=False, process="connect")
        # get account
        ...
        self.assertEqual(
            t.token_secret, json.loads(self.get_login_response_json(with_refresh_token=True)).get("refresh_token", "")
        )
...
class SocialAccountTests(TestCase):
    def setUp(self):
        ...
        for provider in providers.registry.get_list():
            app = SocialApp.objects.create(
                provider=provider.id, name=provider.id, client_id="app123id", key="123", secret="dummy"
            )
            app.sites.add(site)
    ...
    def test_verified_email_change_at_signup(self):
        ...
        self.assertTrue(
            EmailAddress.objects.filter(user=user, email="verified@example.com", verified=True, primary=True).exists()
        )
        self.assertTrue(
    ...
    def test_unverified_email_change_at_signup(self):
        ...
        sociallogin = SocialLogin(
            ...
            account=SocialAccount(provider="google"),
            email_addresses=[EmailAddress(email="unverified@example.com", verified=False, primary=True)],
        )
        session["socialaccount_sociallogin"] = sociallogin.serialize()
        session.save()
        resp = self.client.get(reverse("socialaccount_signup"))
    ...
    def test_unique_email_validation_signup(self):
        ...
        sociallogin = SocialLogin(
            ...
            account=SocialAccount(provider="google"),
            email_addresses=[EmailAddress(email="me@example.com", verified=True, primary=True)],
        )
        session["socialaccount_sociallogin"] = sociallogin.serialize()
        session.save()
        resp = self.client.get(reverse("socialaccount_signup"))
        ...
        resp = self.client.post(reverse("socialaccount_signup"), data={"email": "me@example.com"})
    ...
    def test_social_account_taken_at_signup(self):
        ...
        session = self.client.session
        User = get_user_model()
        sociallogin = SocialLogin(user=User(email="me1@example.com"), account=SocialAccount(provider="facebook"))
        session["socialaccount_sociallogin"] = sociallogin.serialize()
        session.save()
        resp = self.client.get(reverse("socialaccount_signup"))
        form = resp.context["form"]
