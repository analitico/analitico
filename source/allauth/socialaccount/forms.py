...
class SignupForm(BaseSignupForm):
    def __init__(self, *args, **kwargs):
        ...
        kwargs.update({"initial": initial, "email_required": kwargs.get("email_required", app_settings.EMAIL_REQUIRED)})
    ...
    def validate_unique_email(self, value):
        try:
            ...
            raise forms.ValidationError(
                get_adapter().error_messages["email_taken"] % self.sociallogin.account.get_provider().name
            )


class DisconnectForm(forms.Form):
    account = forms.ModelChoiceField(queryset=SocialAccount.objects.none(), widget=forms.RadioSelect, required=True)
