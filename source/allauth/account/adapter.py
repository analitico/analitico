...
from django.contrib.auth import authenticate, get_backends, login as django_login, logout as django_logout
...
from ..utils import build_absolute_uri, email_address_exists, generate_unique_username, get_user_model, import_attribute


class DefaultAccountAdapter(object):

    error_messages = {
        "username_blacklisted": _("Username can not be used. Please use other username."),
    ...
    def get_login_redirect_url(self, request):
        ...
        if url:
            warnings.warn(
                "LOGIN_REDIRECT_URLNAME is deprecated, simply" " use LOGIN_REDIRECT_URL with a URL name",
                DeprecationWarning,
            )
        else:
    ...
    def populate_username(self, request, user):
        ...
        if app_settings.USER_MODEL_USERNAME_FIELD:
            user_username(
                user, username or self.generate_unique_username([first_name, last_name, email, username, "user"])
            )
    ...
    def clean_username(self, username, shallow=False):
        ...
        if not shallow:
            ...
            if filter_users_by_username(username).exists():
                ...
                error_message = user_model._meta.get_field(username_field).error_messages.get("unique")
                if not error_message:
                    error_message = self.error_messages["username_taken"]
                raise forms.ValidationError(
                    error_message, params={"model_name": user_model.__name__, "field_label": username_field}
                )
        return username
    ...
    def ajax_response_form(self, form):
        ...
        for field in form:
            field_spec = {
                ...
                "errors": [force_text(e) for e in field.errors],
                "widget": {"attrs": {k: force_text(v) for k, v in field.field.widget.attrs.items()}},
            }
    ...
    def get_user_search_fields(self):
        ...
        return filter(
            ...
            [app_settings.USER_MODEL_USERNAME_FIELD, "first_name", "last_name", "email"],
        )
    ...
    def _get_login_attempts_cache_key(self, request, **credentials):
        ...
        return "allauth/login_attempts@{site_id}:{login}".format(site_id=site.pk, login=login_key)
