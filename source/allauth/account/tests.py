...
from .utils import filter_users_by_username, url_str_to_user_pk, user_pk_to_url_str, user_username


test_username_validators = [validators.RegexValidator(regex=r"^[a-c]+$", message="not abc", flags=0)]
...
class AccountTests(TestCase):
    ...
    def test_username_containing_at(self):
        ...
        user.save()
        EmailAddress.objects.create(user=user, email="raymond.penners@example.com", primary=True, verified=True)
        resp = self.client.post(reverse("account_login"), {"login": "@raymond.penners", "password": "psst"})
    ...
    def test_signup_same_email_verified_externally(self):
        user = self._test_signup_email_verified_externally("john@example.com", "john@example.com")
        self.assertEqual(EmailAddress.objects.filter(user=user).count(), 1)
        EmailAddress.objects.get(verified=True, email="john@example.com", user=user, primary=True)
    ...
    def test_signup_other_email_verified_externally(self):
        ...
        used as a secondary.
        """
        user = self._test_signup_email_verified_externally("john@example.com", "john@example.org")
        self.assertEqual(EmailAddress.objects.filter(user=user).count(), 2)
        EmailAddress.objects.get(verified=False, email="john@example.com", user=user, primary=False)
        EmailAddress.objects.get(verified=True, email="john@example.org", user=user, primary=True)

    def _test_signup_email_verified_externally(self, signup_email, verified_email):
        username = "johndoe"
        request = RequestFactory().post(
            reverse("account_signup"),
            {"username": username, "email": signup_email, "password1": "johndoe", "password2": "johndoe"},
        )
    ...
    @override_settings(ACCOUNT_USERNAME_REQUIRED=True, ACCOUNT_SIGNUP_PASSWORD_ENTER_TWICE=True)
    def test_signup_password_twice_form_error(self):
        resp = self.client.post(
            reverse("account_signup"),
            data={"username": "johndoe", "email": "john@example.org", "password1": "johndoe", "password2": "janedoe"},
        )
        self.assertFormError(resp, "form", "password2", "You must type the same password each time.")

    @override_settings(ACCOUNT_USERNAME_REQUIRED=True, ACCOUNT_SIGNUP_EMAIL_ENTER_TWICE=True)
    ...
    def test_ajax_password_change(self):
        ...
        resp = self.client.post(
            ...
            data={"oldpassword": "doe", "password1": "AbCdEf!123", "password2": "AbCdEf!123456"},
    ...
    def test_password_reset_flow(self):
        ...
        response = self.client.post(
            url, {"password1": "newpass123", "password2": "newpass123"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    def test_email_verification_mandatory(self):
        ...
        resp = c.post(
            reverse("account_signup"),
            {"username": "johndoe", "email": "john@example.com", "password1": "johndoe", "password2": "johndoe"},
            follow=True,
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_login_unverified_account_optional(self):
        ...
        user.set_password("doe")
        user.save()
        EmailAddress.objects.create(user=user, email="user@example.com", primary=True, verified=False)
        resp = self.client.post(reverse("account_login"), {"login": "john", "password": "doe"})
        self.assertRedirects(resp, settings.LOGIN_REDIRECT_URL, fetch_redirect_response=False)

    @override_settings(
        ACCOUNT_EMAIL_VERIFICATION=app_settings.EmailVerificationMethod.OPTIONAL, ACCOUNT_LOGIN_ATTEMPTS_LIMIT=3
    )
    def test_login_failed_attempts_exceeded(self):
        ...
        for i in range(5):
            ...
            is_locked = i >= 3
            resp = self.client.post(
                reverse("account_login"), {"login": "john", "password": ("doe" if is_valid_attempt else "wrong")}
            )
    ...
    def test_login_inactive_account(self):
        ...
        user.set_password("doe")
        user.save()
        EmailAddress.objects.create(user=user, email="john@example.com", primary=True, verified=True)
        resp = self.client.post(reverse("account_login"), {"login": "john", "password": "doe"})
        ...
        user.save()
        EmailAddress.objects.create(user=user, email="user@example.com", primary=True, verified=False)
        resp = self.client.post(reverse("account_login"), {"login": "doe", "password": "john"})
        self.assertRedirects(resp, reverse("account_inactive"))
    ...
    def test_ajax_login_success(self):
        ...
        user.save()
        resp = self.client.post(
            reverse("account_login"), {"login": "john", "password": "doe"}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.content.decode("utf8"))
    ...
    def test_django_password_validation(self):
        ...
        self.assertFormError(
            resp, "form", "password1", ["This password is too short." " It must contain at least 9 characters."]
        )
    ...
    @override_settings(ACCOUNT_USERNAME_VALIDATORS="allauth.account.tests" ".test_username_validators")
...
class EmailFormTests(TestCase):
    ...
    def test_add(self):
        resp = self.client.post(reverse("account_email"), {"action_add": "", "email": "john3@example.org"})
        EmailAddress.objects.get(email="john3@example.org", user=self.user, verified=False, primary=False)
        self.assertTemplateUsed(resp, "account/messages/email_confirmation_sent.txt")

    def test_ajax_get(self):
        resp = self.client.get(reverse("account_email"), HTTP_X_REQUESTED_WITH="XMLHttpRequest")
        data = json.loads(resp.content.decode("utf8"))
        assert data["data"] == [
            {"id": self.email_address.pk, "email": "john1@example.org", "primary": True, "verified": True},
            {"id": self.email_address2.pk, "email": "john2@example.org", "primary": False, "verified": False},
        ]
    ...
    def test_remove_primary(self):
        resp = self.client.post(reverse("account_email"), {"action_remove": "", "email": self.email_address.email})
        EmailAddress.objects.get(pk=self.email_address.pk)
        self.assertTemplateUsed(resp, "account/messages/cannot_delete_primary_email.txt")
    ...
    def test_remove_secondary(self):
        resp = self.client.post(reverse("account_email"), {"action_remove": "", "email": self.email_address2.email})
        self.assertRaises(EmailAddress.DoesNotExist, lambda: EmailAddress.objects.get(pk=self.email_address2.pk))
        self.assertTemplateUsed(resp, "account/messages/email_deleted.txt")

    def test_set_primary_unverified(self):
        resp = self.client.post(reverse("account_email"), {"action_primary": "", "email": self.email_address2.email})
    ...
    def test_verify(self):
        resp = self.client.post(reverse("account_email"), {"action_send": "", "email": self.email_address2.email})
        self.assertTemplateUsed(resp, "account/messages/email_confirmation_sent.txt")


class BaseSignupFormTests(TestCase):
    @override_settings(ACCOUNT_USERNAME_REQUIRED=True, ACCOUNT_USERNAME_BLACKLIST=["username"])
    ...
    def test_signup_email_verification(self):
        ...
        data = {"username": "username", "email": "user@example.com", "email2": "user@example.com"}
        form = BaseSignupForm(data, email_required=True)
        self.assertTrue(form.is_valid())
...
class CustomSignupFormTests(TestCase):
    @override_settings(ACCOUNT_SIGNUP_EMAIL_ENTER_TWICE=True, ACCOUNT_SIGNUP_PASSOWRD_ENTER_TWICE=True)
    def test_custom_form_field_order(self):

        expected_field_order = ["email", "email2", "password1", "password2", "username", "last_name", "first_name"]
...
class AuthenticationBackendTests(TestCase):
    def setUp(self):
        user = get_user_model().objects.create(is_active=True, email="john@example.com", username="john")
        user.set_password(user.username)
        user.save()
        self.user = user

    @override_settings(ACCOUNT_AUTHENTICATION_METHOD=app_settings.AuthenticationMethod.USERNAME)  # noqa
    def test_auth_by_username(self):
        user = self.user
        backend = AuthenticationBackend()
        self.assertEqual(backend.authenticate(request=None, username=user.username, password=user.username).pk, user.pk)
        self.assertEqual(backend.authenticate(request=None, username=user.email, password=user.username), None)

    @override_settings(ACCOUNT_AUTHENTICATION_METHOD=app_settings.AuthenticationMethod.EMAIL)  # noqa
    def test_auth_by_email(self):
        user = self.user
        backend = AuthenticationBackend()
        self.assertEqual(backend.authenticate(request=None, username=user.email, password=user.username).pk, user.pk)
        self.assertEqual(backend.authenticate(request=None, username=user.username, password=user.username), None)

    @override_settings(ACCOUNT_AUTHENTICATION_METHOD=app_settings.AuthenticationMethod.USERNAME_EMAIL)  # noqa
