...
class EmailView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    def _action_send(self, request, *args, **kwargs):
        ...
        try:
            ...
            get_adapter(request).add_message(
                request, messages.INFO, "account/messages/" "email_confirmation_sent.txt", {"email": email}
            )
    ...
    def _action_remove(self, request, *args, **kwargs):
        ...
        try:
            ...
            if email_address.primary:
                get_adapter(request).add_message(
                    request, messages.ERROR, "account/messages/" "cannot_delete_primary_email.txt", {"email": email}
                )
            else:
                email_address.delete()
                signals.email_removed.send(
                    sender=request.user.__class__, request=request, user=request.user, email_address=email_address
                )
                get_adapter(request).add_message(
                    request, messages.SUCCESS, "account/messages/email_deleted.txt", {"email": email}
                )
                return HttpResponseRedirect(self.get_success_url())
        except EmailAddress.DoesNotExist:
            pass
    ...
    def _action_primary(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not email_address.verified and EmailAddress.objects.filter(user=request.user, verified=True).exists():
                get_adapter(request).add_message(
                    request, messages.ERROR, "account/messages/" "unverified_primary_email.txt"
                )
            else:
                ...
                get_adapter(request).add_message(request, messages.SUCCESS, "account/messages/primary_email_set.txt")
...
class PasswordSetView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    def render_to_response(self, context, **response_kwargs):
        return super(PasswordSetView, self).render_to_response(context, **response_kwargs)
    ...
    def form_valid(self, form):
        ...
        signals.password_set.send(sender=self.request.user.__class__, request=self.request, user=self.request.user)
...
class PasswordResetFromKeyView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ret["action_url"] = reverse(
            "account_reset_password_from_key", kwargs={"uidb36": self.kwargs["uidb36"], "key": self.kwargs["key"]}
        )
        return ret
...
class LogoutView(TemplateResponseMixin, View):
    ...
    def get_context_data(self, **kwargs):
        ctx = kwargs
        redirect_field_value = get_request_param(self.request, self.redirect_field_name)
        ctx.update({"redirect_field_name": self.redirect_field_name, "redirect_field_value": redirect_field_value})
        return ctx

    def get_redirect_url(self):
        return get_next_redirect_url(self.request, self.redirect_field_name) or get_adapter(
            self.request
