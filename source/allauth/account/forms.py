...
class LoginForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if app_settings.AUTHENTICATION_METHOD == AuthenticationMethod.EMAIL:
            login_widget = forms.TextInput(
                attrs={"type": "email", "placeholder": _("E-mail address"), "autofocus": "autofocus"}
            )
            ...
            login_widget = forms.TextInput(attrs={"placeholder": _("Username or e-mail"), "autofocus": "autofocus"})
    ...
    def login(self, request, redirect_url=None):
        ret = perform_login(
            request, self.user, email_verification=app_settings.EMAIL_VERIFICATION, redirect_url=redirect_url
        )
...
def _base_signup_form_class():
    ...
    if not hasattr(fc_class, "signup"):
        if hasattr(fc_class, "save"):
            warnings.warn(
                "The custom signup form must offer" " a `def signup(self, request, user)` method", DeprecationWarning
            )
        else:
...
class BaseSignupForm(_base_signup_form_class()):
    ...
    def __init__(self, *args, **kwargs):
        ...
        username_field.validators.append(validators.MaxLengthValidator(username_field.max_length))
        ...
        if app_settings.SIGNUP_EMAIL_ENTER_TWICE:
            self.fields["email2"] = forms.EmailField(
                ...
                widget=forms.TextInput(attrs={"type": "email", "placeholder": _("E-mail address confirmation")}),
            )
        if email_required:
        ...
        set_form_field_order(self, getattr(self, "field_order", None) or default_field_order)
...
class AddEmailForm(UserForm):
    ...
    email = forms.EmailField(
        label=_("E-mail"),
        required=True,
        widget=forms.TextInput(attrs={"type": "email", "size": "30", "placeholder": _("E-mail address")}),
    )
    ...
    def save(self, request):
        return EmailAddress.objects.add_email(request, self.user, self.cleaned_data["email"], confirm=True)
